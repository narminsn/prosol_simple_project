...
jQuery(function ($) {
    ...
    function next(button, group, show, hide) {
        ...
        $(document).on('click', button, function () {
            ...
            var curStep = $(this).parent().closest("fieldset")
            console.log(curStep,'step')
            var isValid = true;
            var curInputs = curStep.find("input[type='text'],input[type='phone'],input[type='email']")
            for (var i = 0; i < curInputs.length; i++) {
                if (!curInputs[i].validity.valid) {
                    isValid = false;
                    // $(curInputs[i]).closest(".form-group").addClass("has-error");
                    // $(curInputs[i]).closest("select").addClass("has-error");
                }
            }
